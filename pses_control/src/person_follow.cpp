...
/*
 * NAME:			detectOrTrack
 * DESCRIPTION:		Organizes sequence of tasks for goal creation: Switches between detecting and tracking
					and performs clustering of contents of the bounding box followed by a controller.
					Published outputs of controller.
...
void detectOrTrack(Mat frame) {
	...
	// Start detection if bounding box does not exist or some tracking cycles have passed
    if (bbox_.tl() == bbox_.br() || cnt_ > 100) {
		...
		// Modify bounding box size for adapting to distorted depth image
	...
	// Set manipulated values to zero if no target has been received
...
/*
 * NAME:			colorImageCallback
 * DESCRIPTION:		Convert ROS image to OpenCV Mat
 * INPUT:			const sensor_msgs::ImageConstPtr& color_image
 * OUTPUT: 			
*/
 ...
 * DESCRIPTION:		Reconfigure variables by using ROS's dynamic reconfigure
 * INPUT:			const sensor_msgs::ImageConstPtr& color_image
 * OUTPUT: 			
*/
